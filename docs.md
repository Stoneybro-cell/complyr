...
### LayerZero DVN instability

The cross-chain compliance bridge relies on LayerZero V2 testnet DVNs (Decentralised Verifier Networks) to relay messages from Flow to Zama Sepolia. Testnet DVNs can be slow, intermittently unavailable, or silently drop messages without reversion. In production, the message queue is persistent — a failed delivery can always be retried — but on testnet you may encounter delays of minutes to hours.

As a workaround, Complyr's relay API (`/api/relay/compliance-record`) submits compliance records directly to the Zama registry without going through LayerZero, bypassing the bridge entirely for demo reliability. The bridge path remains the canonical production design.
