...
**3. Handle and proof extraction.** The resulting ciphertext handle (32 bytes) and zero-knowledge proof are encoded as hex strings and sent to be recorded on the registry. In production architecture, encrypted compliance payloads are delivered via the LayerZero bridge. For demo reliability on testnet, a relay API can submit records directly to the registry.
