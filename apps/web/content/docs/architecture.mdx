...
**4. Cross-chain compliance dispatch.** In the production architecture, the Compliance Bridge — a LayerZero V2 OApp deployed on Flow EVM — encodes the compliance report and sends it to Zama Sepolia. For testnet reliability, a relay API can submit records directly to the registry, bypassing LayerZero.
