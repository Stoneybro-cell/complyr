...
description: Deep dive into the HashKey Chain smart contracts.
---

## Smart Contract Layer — HashKey Chain

The HashKey Chain layer handles payment execution and compliance metadata collection. It consists of these main contracts.
...
The factory also triggers automatic company registration on the ComplianceRegistry at wallet creation time. This establishes the identity link mapping the HashKey proxy address to the owner's EOA at the earliest possible moment.
...
- `txHash` — the deterministic link to the HashKey transaction or intent
